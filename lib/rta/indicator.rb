module Rta
  ...
  class Indicator
    ...
    def moving_averages(subset_length: nil, period: nil)
      Rta::MovingAverages.new(series: extract_series(subset_length: subset_length), period: period)
    end

    def statistical_methods(subset_length: nil)
      Rta::StatisticalMethods.new(series: extract_series(subset_length: subset_length))
    end
  end
end
